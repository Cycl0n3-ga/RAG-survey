...
> [!IMPORTANT]
> 本資料夾的正式 RAG taxonomy 為 **14 個 Research Domains（D01–D14）**。

## Structure

```text
Research Domains
└── Canonical RAG Domains
...
- [[00 - 導覽與心智圖 (Navigation & MOC)/RAG Paradigm Tags|Paradigm Tags]]：GraphRAG、Hierarchical RAG、Agentic RAG、Multimodal RAG 等跨 Domain 方法族。
- [[00 - 導覽與心智圖 (Navigation & MOC)/RAG Adjacent Interfaces|Adjacent Interfaces]]：Long Context、KV Cache、General Agents 等相鄰研究。
- [[00 - 導覽與心智圖 (Navigation & MOC)/RAG Research Taxonomy & Domain Map|Taxonomy & Domain Map]]：完整分類規則與邊界。
- [[00 - 導覽與心智圖 (Navigation & MOC)/LLM 超長文件處理心智圖 (MOC)|System Maps]]：核心資料流與可選流程圖。
